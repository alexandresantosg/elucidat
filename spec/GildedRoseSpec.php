...
describe('Gilded Rose', function () {
    describe('next day', function () {
        ...
        context('Sulfuras Items', function () {
            ...
            it('sulfuras maintains its quality even when it is bigger than 80 before sell date', function () {
                $gr = new GildedRose([new Item('Sulfuras, Hand of Ragnaros', 80, 10)]);
                $gr->nextDay();
                expect($gr->getItem(0)->quality)->toBe(80);
                expect($gr->getItem(0)->sellIn)->toBe(10);
            });
            it('sulfuras maintains its quality even when it is bigger than 80 on sell date', function () {
                $gr = new GildedRose([new Item('Sulfuras, Hand of Ragnaros', 80, 0)]);
                $gr->nextDay();
                expect($gr->getItem(0)->quality)->toBe(80);
                expect($gr->getItem(0)->sellIn)->toBe(0);
            });
            it('sulfuras maintains its quality even when it is bigger than 80 after sell date', function () {
                $gr = new GildedRose([new Item('Sulfuras, Hand of Ragnaros', 80, -1)]);
                $gr->nextDay();
                expect($gr->getItem(0)->quality)->toBe(80);
                expect($gr->getItem(0)->sellIn)->toBe(-1);
            });
